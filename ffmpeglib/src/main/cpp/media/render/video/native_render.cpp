...
void NativeRender::InitRender(int video_width, int video_height, int *dst_size) {
    // 绘制区域的宽高
    ...
    // 计算目标视频的宽高
    m_dst_w = windowWidth;
    m_dst_h = windowWidth * video_height / video_width;
    if (m_dst_h > windowHeight) {
        m_dst_h = windowHeight;
        m_dst_w = windowHeight * video_width / video_height;
    }
    LOGD("NativeRender::windowW: %d, windowH: %d, dstVideoW: %d, dstVideoH: %d",
...
void NativeRender::RenderVideoFrame(OneFrame *one_frame) {
//锁定窗口
...
void NativeRender::UnInitRender() {
    if (m_native_window) {
