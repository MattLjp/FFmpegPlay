...
OpenGLRender::OpenGLRender(JNIEnv *env, DrawerProxy *drawer_proxy, jobject surface) :
...
void OpenGLRender::SetScreenSize(int width, int height) {
...
void OpenGLRender::InitRender(int video_width, int video_height, int *dst_size) {
    m_drawer_proxy->SetDrawerSize(video_width, video_height);
    dst_size[0] = video_width;
    dst_size[1] = video_height;
}

void OpenGLRender::RenderVideoFrame(OneFrame *one_frame) {
    m_drawer_proxy->Render(one_frame->data);
}

void OpenGLRender::UnInitRender() {
    m_state = STOP;
}
