...
void FFRepack::Init(const char *in_path, const char *out_path) {
    // 打开原视频文件，并获取相关参数
    if (OpenSrcFile(in_path) >= 0) {
        LOGE("FfRepack::Open src file success")
        // 初始化打包参数
        if (InitMuxerParams(out_path) >= 0) {
            LOGE("FfRepack::InitFFEncoder muxer params success")
        } else {
            LOGE("FfRepack::InitFFEncoder muxer params fail")
        }
    } else {
        LOGE("FfRepack::Open src file fail")
    }
}

int FFRepack::OpenSrcFile(const char *srcPath) {
    // 打开文件
    LOGI("FfRepack::Open file: %s", srcPath)
    if ((avformat_open_input(&m_in_format_cxt, srcPath, NULL, NULL)) < 0) {
        LOGE("FfRepack::Fail to open input file")
        return -1;
    }

    // 获取音视频参数
    if ((avformat_find_stream_info(m_in_format_cxt, 0)) < 0) {
        LOGE("FfRepack::Fail to retrieve input stream information")
        return -1;
    }
...
int FFRepack::InitMuxerParams(const char *destPath) {
    ...
    for (int i = 0; i < m_in_format_cxt->nb_streams; ++i) {
        ...
        if (!out_stream) {
            LOGE("FfRepack::Fail to allocate output stream")
            return -1;
        }

        // 复制原视频数据流参数到目标输出流
        if (avcodec_parameters_copy(out_stream->codecpar, in_stream->codecpar) < 0) {
            LOGE("FfRepack::Fail to copy input context to output stream")
            return -1;
        }
    }
    ...
    if (avio_open(&m_out_format_cxt->pb, destPath, AVIO_FLAG_WRITE) < 0) {
        LOGE("FfRepack::Could not open output file %s ", destPath);
        return -1;
    }

    // 写入文件头信息
    if (avformat_write_header(m_out_format_cxt, NULL) < 0) {
        LOGE("FfRepack::Error occurred when opening output file");
        return -1;
    } else {
        LOGE("FfRepack::Write file header success");
    }
...
void FFRepack::Start() {
    LOGE("FfRepack::Start repacking ....")
    AVPacket pkt;
    while (1) {
        // 读取数据
        if (av_read_frame(m_in_format_cxt, &pkt)) {
            LOGE("FfRepack::End of video，write trailer")
...
void FFRepack::Write(AVPacket pkt) {
    ...
    if (ret < 0) {
        LOGE("FfRepack::Error to muxing packet: %x", ret)
    }
}

void FFRepack::Release() {
    LOGE("FfRepack::Finish repacking, release resources")
    // 关闭输入
    if (m_in_format_cxt) {
