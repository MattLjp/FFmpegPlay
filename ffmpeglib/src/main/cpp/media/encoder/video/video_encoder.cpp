...
VideoEncoder::VideoEncoder(Mp4Muxer *muxer, int width, int height) {
    m_width = width;
    m_height = height;
    Init(muxer, AV_CODEC_ID_H264);
}

void VideoEncoder::InitContext(AVCodecContext *codec_ctx) {
    m_sws_ctx = sws_getContext(m_width, m_height, AV_PIX_FMT_RGBA,
                               m_width, m_height, AV_PIX_FMT_YUV420P, SWS_FAST_BILINEAR,
                               NULL, NULL, NULL);
...
//    codec_ctx->codec_id = AV_CODEC_ID_H264;
    ...
    LOGI("VideoEncoder::Init codec context success")
}
...
void VideoEncoder::InitYUVFrame() {
    ...
    if (ret < 0) {
        LOGE("VideoEncoder::Fail to get yuv frame buffer");
    }
}
